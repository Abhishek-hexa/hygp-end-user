...
export type FeatureTabsHeaderHandle = {
  scrollToFeature: (feature: Features) => void;
};

export const FeatureTabsHeader = observer(
  forwardRef<FeatureTabsHeaderHandle, FeatureTabsHeaderProps>(
    (
      { activeFeature, features, onSelectFeature, scrollToStartSignal },
      ref,
    ) => {
      ...
      // ✅ Expose scrollToFeature to parent
      useImperativeHandle(ref, () => ({
        scrollToFeature(feature: Features) {
          requestAnimationFrame(() => {
            const container = scrollRef.current;
            if (!container) return;

            const tabs =
              container.querySelectorAll<HTMLElement>('[data-feature]');
      ...
      useEffect(() => {
        if (scrollToStartSignal === 0) return;
        const container = scrollRef.current;
        if (!container) return;
